...
class PromptTemplate:
    ...
    def __init__(self,
        ...
        # 检查 self.selected_column_name 和 self.selected_column_map 是否同时 设置
    ...
    def _check_template_legacy(self):
        ...
        # Check duplicates
        if len(self.column_token_map.values()) != len(set(self.column_token_map.values())):  # 确保 模板字典 的 值不重复
            raise ValueError(f"There are duplicates in self.column_token_map.values()")
        if self.ice_token is not None and self.ice_token in self.column_token_map.values():  # 确保 ice 占位符 不与 列名占位符 重复
    ...
    def generate_item(self, entry: Dict, output_field: Optional[Hashable] = None,
        ...
        # 选择 合适的模板
        if isinstance(self.template, str):
            tp = self.template  # 模板是 字符串，直接赋值
        else:  # 模板是字典，根据 label 选择 相应的模板str
            pred_label = None
        ...
        for key, token in self.column_token_map.items():
            if output_field is not None and key == output_field:
                ...
                tp = tp.replace(token, str(entry[key]))  # 如果 tp 中没有 token， 就不会替换，因此 逻辑没问题
        return tp
